...
def extract_logs(url):
    ...
    if response.status_code == 200:
        ...
        print(f"Ошибка при получении: HTTP {response.status_code}")
        return None

def prs_log_line(log_line):
...
def paste_logs_to_db(db_cursor, log_data):
    db_cursor.execute('''
...
def dlt_table(db_cursor):
    db_cursor.execute('DROP TABLE IF EXISTS apache_logs')

def rcv_logs_from_db(db_cursor, table_name):
    db_cursor.execute(f"SELECT * FROM {table_name}")
    rows = db_cursor.fetchall()
    return rows

def fltr_data(rows, filter_values):
    filtered_logs = []
    date_range = []
    filter_date = []
